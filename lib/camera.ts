...
//      this.unloadDriver();
  ...
  startRtsp() {
    ...
    if (this.config.MulticastEnabled) {
        ...
        if (this.config.RTSPServer == 4) this.rtspServer = utils.spawn("./subprojects/OnvifRtspLauncher/build/onvifserver", ["-p",this.config.RTSPPort.toString(), "-m" , this.config.RTSPName.toString(),"-w",this.settings.resolution.Width.toString(),"-h",this.settings.resolution.Height.toString(), "-v", (this.config.CameraType == "testsrc" ? "test" : this.config.CameraDevice), "-a", (this.config.CameraType == "testsrc" ? "test" : ""), "-c", this.config.Codec, "-f", this.config.FPS.toString()]);
    }
