...
#include <sys/shm.h>
#include <cstring>
#define VIDEO_OUT "/dev/video4" // V4L2 Loopack / Thermal
#define VIDEO_OUT_VISIBLE "/dev/video6" // V4L2 Loopack / Visible
...
int main ( int argc, char **argv ) {
    ...
    struct v4l2_format vid_format_visible;
    ...
    if ( (fd_visible = open ( VIDEO_OUT_VISIBLE, O_RDWR )) == -1 )
        printf ("Unable to open video output!");

    memset ( &vid_format, 0, sizeof(vid_format) );
    memset ( &vid_format_visible, 0, sizeof(vid_format_visible) );
    // std::cout << &vid_format << std::endl;
    vid_format.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
    vid_format_visible.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;

    if ( ioctl ( fd, VIDIOC_G_FMT, &vid_format ) == -1 )
        printf ( "Unable to get video format data. Errro: %d\n", errno );

    if ( ioctl ( fd_visible, VIDIOC_G_FMT, &vid_format_visible ) == -1 )
        printf ( "Unable to get video format data. Errro: %d\n", errno );
    ...
    vid_format_visible.fmt.pix.width       = cap.get ( CV_CAP_PROP_FRAME_WIDTH  );
    vid_format_visible.fmt.pix.height      = cap.get ( CV_CAP_PROP_FRAME_HEIGHT );
    vid_format_visible.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV420;
    
    vid_format_visible.fmt.pix.sizeimage   = framesize;
    vid_format_visible.fmt.pix.field       = V4L2_FIELD_NONE;

    if ( ioctl ( fd, VIDIOC_S_FMT, &vid_format ) == -1 )
        printf ( "Unable to set video format! Errno: %d\n", errno );

    if ( ioctl ( fd_visible, VIDIOC_S_FMT, &vid_format_visible ) == -1 )
        printf ( "Unable to set video format! Errno: %d\n", errno );

    cv::Mat frame ( cap.get(CV_CAP_PROP_FRAME_HEIGHT), cap.get(CV_CAP_PROP_FRAME_WIDTH), CV_8UC3 );
    cv::Mat frame_visible ( cap.get(CV_CAP_PROP_FRAME_HEIGHT), cap.get(CV_CAP_PROP_FRAME_WIDTH), CV_8UC3 );
    cv::Mat frameROI_0;
    ...
    while (1) {
        ...
        cv::cvtColor ( frame_visible, frame_visible, cv::COLOR_BGR2YUV_IYUV);
        write ( fd, frame.data, framesize );
        write ( fd_visible, frame_visible.data, framesize );
    }
}
////////
...
// sudo modprobe bcm2835-v4l2
// sudo modprobe v4l2loopback video_nr=4,5,6,7
// g++ -ggdb `pkg-config --cflags --libs opencv` i3demo.cpp -o i3demo
// v4l2compress -fH264 /dev/video4 /dev/video5
// v4l2compress -fH264 /dev/video6 /dev/video7